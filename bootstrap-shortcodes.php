<?php
/*
Plugin Name: Complete Bootstrap 5 Shortcodes
Plugin URI: https://github.com/uwejacobs/complete-bootstrap-5-shortcodes
Description: The plugin adds shortcodes for most Bootstrap 5 elements.
Version: 5.0.0
Author: Uwe Jacobs
Author URI:
License: MIT
...
class BootstrapShortcodes {
    ...
    function bs_button($save_atts, $content = null) {
        ...
        $id = 'bs5-button-' . $GLOBALS['button_count'];
        ...
        $button_data = array();
        $button_data[] = ($this->is_flag('dropdown', $save_atts)) ? 'bs-toggle,dropdown' : '';
        if ($atts['modal']) {
            $button_data[] = 'bs-toggle,modal';
            $button_data[] = 'bs-target,#' . $atts['modal'];
        }
	...
	if ($this->is_flag('block', $save_atts)) {
            $return = '<div class="d-grid">' . $return . '</div>';
	}
    ...
    function bs_tag($save_atts, $content = null) {
        ...
        return sprintf('<%s%s%s>%s</%s>', $atts["name"], $this->class_output($class, $atts["class"]) , $data_props, do_shortcode($content), $atts["name"]);
    }
    ...
    function bs_dropdown_menu($save_atts, $content = null) {
        ...
        $class = array();
        $class[] = 'dropdown-menu';
        $class[] = ($this->is_flag('right', $save_atts) || $this->is_flag('end', $save_atts)) ? 'dropdown-menu-end' : '';
    ...
    function bs_nav_item($save_atts, $content = null) {
        ...
        return sprintf('<li%1$s><a href="%2$s"%3$s%4$s%5$s%6$s>%7$s</a></li>', $this->class_output($li_class) , esc_url($atts['link']) , $this->class_output($a_class, $atts["class"]) , ($this->is_flag('dropdown', $save_atts)) ? ' data-bs-toggle="dropdown"' : '', $data_props, $a_aria, do_shortcode($content));
    ...
    function bs_accordion($save_atts, $content = null) {
        ...
        $class = array();
        $class[] = "accordion";
        $class[] = ($this->is_flag('flush', $save_atts)) ? 'accordion-flush' : '';
    ...
    function bs_card($save_atts, $content = null) {
        ...
        $class = array();
        $class[] = isset($GLOBALS['accordion']) ? 'accordion-item' : 'card';

        $data_props = $this->parse_data_attributes($atts['data']);

	$return = sprintf('<div%s%s>%s</div>', $this->class_output($class, $atts["class"]) , $data_props, do_shortcode($content));
	if (isset($GLOBALS['card-deck'])) {
            $return = '<div class="col">' . $return . '</div>';
	}
	return $return;
    }
    ...
    function bs_card_body($save_atts, $content = null) {
        ...
        $wrap_before = (isset($GLOBALS['accordion'])) ? sprintf('<div id="collapse%s" class="accordion-collapse collapse%s" data-bs-parent="#accordion%s" aria-labelledby="bs-heading%s">', $GLOBALS['accordion_card'], $show, $GLOBALS['accordion_count'], $GLOBALS['accordion_card']) : '';
        $wrap_after = (isset($GLOBALS['accordion'])) ? '</div>' : '';

        $class = array();
        $class[] = isset($GLOBALS['accordion']) ? 'accordion-body' : 'card-body';

        $p_class = array();
        $p_class[] = isset($GLOBALS['accordion']) ? 'accordion-text' : 'card-text';
        $p_tags = array(
            'p'
        );
    ...
    function bs_card_header($atts, $content = null) {
        ...
        $wrap_before .= ($this->testdom($content, $search_tags)) ? '' : '<div class="' . (isset($GLOBALS['accordion']) ? 'accordion' : 'card') . '-header"' . $id . '>';
        $wrap_after .= ($this->testdom($content, $search_tags)) ? '' : '</div>';

        $wrap_before .= (isset($GLOBALS['accordion'])) ? sprintf('<button class="accordion-button%1$s" type="button" data-bs-toggle="collapse" data-bs-target="#collapse%2$s" aria-controls="collapse%2$s" aria-expanded="%3$s">', $GLOBALS['accordion_card_show'] ? '' : ' collapsed', $GLOBALS['accordion_card'], $GLOBALS['accordion_card_show'] ? 'true' : 'false') : '';
	...
	$GLOBALS['card-deck'] = true;
        $return = sprintf('<div%s%s>%s</div>', $this->class_output($class, $atts["class"]) , $data_props, do_shortcode($content));
	unset($GLOBALS['card-deck']);
    ...
    function bs_alert($save_atts, $content = null) {
        ...
        return sprintf('<div role="alert"%s%s>%s%s</div>', $this->class_output($class, $atts["class"]) , $data_props, do_shortcode($content), $dismissible);
    }
    ...
    function bs_column($atts, $content = null) {
        ...
        $class[] = ($atts['offset-xxl'] || $atts['offset-xxl'] === "0") ? 'offset-xxl-' . $atts['offset-xxl'] : '';
    ...
    function bs_badge($save_atts, $content = null) {
        ...
        $class[] = 'badge';
        $class[] = 'bg-' . $atts['bg'];
        $class[] = ($this->is_flag('right', $save_atts) || $this->is_flag('end', $save_atts)) ? 'float-end' : '';
    ...
    function bs_carousel($save_atts, $content = null) {
        ...
        $id = 'bs5-carousel-' . $GLOBALS['carousel_count'];
        ...
        $controls = '';
        $controls .= '<a class="carousel-control-prev" href="' . esc_url('#' . $id) . '" role="button" data-bs-slide="prev"><span class="carousel-control-prev-icon" aria-hidden="true"></span><span class="sr-only">Previous</span></a>';
        $controls .= '<a class="carousel-control-next" href="' . esc_url('#' . $id) . '" role="button" data-bs-slide="next"><span class="carousel-control-next-icon" aria-hidden="true"></span><span class="sr-only">Next</span></a>';

        $indicators = array();
        $i = 0;
        $cnt = $this->counttags('carousel-item', $content);
        while ($i < $cnt) {
            $indicators[] = sprintf('<li data-bs-target="%s" data-bs-slide-to="%s"></li>', esc_attr('#' . $id) , esc_attr($i));
            $i++;
        }
        ...
        $return = sprintf('<div%s id="%s" data-bs-ride="carousel"%s%s%s%s>%s<div class="carousel-inner" role="listbox">%s</div>%s</div>', $this->class_output($class, $atts['class']) , esc_attr($id) , ($atts['interval']) ? sprintf(' data-bs-interval="%d"', $atts['interval']) : '', ($atts['pause']) ? sprintf(' data-bs-pause="%s"', esc_attr($atts['pause'])) : '', ($atts['wrap']) ? sprintf(' data-bs-wrap="%s"', esc_attr($atts['wrap'])) : '', $data_props, ($this->is_flag('indicators', $save_atts)) ? '<ol class="carousel-indicators">' . implode($indicators) . '</ol>' : '', $content, ($this->is_flag('controls', $save_atts)) ? $controls : '');
    ...
    function bs_tooltip($save_atts, $content = null) {
        ...
        $tooltip_data = array();
        $tooltip_data[] = "bs-toggle,tooltip";
        $tooltip_data[] = "bs-placement," . $atts['placement'];
        $tooltip_data[] = ($this->is_flag('html', $save_atts)) ? 'bs-html,true' : '';
        $tooltip_data[] = ($this->is_flag('animation', $save_atts)) ? 'bs-animation,true' : '';
    ...
    function bs_popover($save_atts, $content = null) {
        ...
        $popover_data = array();
        $popover_data[] = "bs-toggle,popover";
        $popover_data[] = "bs-placement," . $atts['placement'];
        $popover_data[] = "bs-container," . $atts['container'];
        $popover_data[] = "bs-content," . $atts['content'];
        $popover_data[] = "bs-trigger," . $atts['trigger'];
        $popover_data[] = ($this->is_flag('html', $save_atts)) ? 'bs-html,true' : '';
        $popover_data[] = ($this->is_flag('animation', $save_atts)) ? 'bs-animation,true' : '';
	...
	$return = '<div' . $this->class_output($class, $atts["class"]) . $this->parse_data_attributes($atts['data']) . '>' . $return . '</div>';
	...
	$class = array();
        ...
        $class[] = 'p-5';
        $class[] = 'bg-light';
        $class[] = ($this->is_flag('fluid', $save_atts)) ? 'container-fluid' : '';
    ...
    function bs_embed_responsive($atts, $content = null) {
        ...
        $class = array();
        $class[] = 'ratio';
        $class[] = ($atts['ratio']) ? 'ratio-' . $atts['ratio'] : '';
    ...
    function bs_modal($save_atts, $content = null) {
        ...
        $return = sprintf('<div id="%s"%s data-bs-backdrop="%s" aria-labelledby="%s" aria-hidden="true" tabindex="-1" role="dialog" %s>
            <div%s role="document">
    ...
    function bs_float($atts, $content = null) {
        $atts = array_change_key_case((array)$atts, CASE_LOWER);
        $atts = shortcode_atts(array(
            "float" => "none", // start, end, left, right, end
            "class" => false,
            "data" => false
        ) , $atts);
    ...
    function close_icon($dismiss) {
        ...
        return '<button type="button" class="btn-close" data-bs-dismiss="' . $dismiss . '" aria-label="Close"></button>';
    }
    ...
    */
    function addDemoPage() {
        ...
        $slug = 'bootstrap-5-shortcodes-demo-' . $version;

        $post_exists = get_page_by_path($slug, OBJECT, 'page');

        if (!$post_exists) {
            $post = array(
                'post_title' => 'Bootstrap 5 Shortcodes Demo ' . $plugin_data["Version"],
                'post_name' => $slug,
